...
void my_qsort(int* sort_system, size_t elements, size_t size, int COMPARE( int a,  int b));
...
void my_qsort(int* sort_system, size_t elements, size_t size, int COMPARE( int a,  int b))
{
    int half_symb = sort_system[elements / 2];

    printf("%d %zu\n", half_symb, elements / 2);
    ...
    do
    {
        while(COMPARE(sort_system[ptr_1], half_symb) < 0)
        {
            ptr_1++;
        }
        while(COMPARE(sort_system[ptr_2], half_symb) > 0)
        {
            ptr_2--;
        }
